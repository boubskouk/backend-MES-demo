    ...
    try {
        ...
        if (maintenanceSettings && maintenanceSettings.enabled === true) {
            ...
            if (!isWhitelisted) {
                ...
                console.log(`🔒 CONNEXION BLOQUÉE (MAINTENANCE): ${username} - Mode maintenance activé par ${maintenanceSettings.enabledBy}`);
        ...
        await new Promise((resolve, reject) => {
            req.session.save(async (err) => {
                if (err) {
                    ...
                    // ✅ CORRECTION: Stocker le sessionID dans l'utilisateur pour coherence
                    const { getCollections } = require('../config/database');
                    const collections = getCollections();
                    ...
                    console.log(`✅ Session créée pour: ${username} (niveau ${req.session.userLevel}, sessionID: ${req.sessionID})`);
                    resolve();
                }
            });
        });
...
async function logout(req, res) {
    try {
        ...
        if (username) {
            ...
            // ✅ Logger la déconnexion dans les audit logs
            const { getCollections } = require('../config/database');
            const collections = getCollections();
            await collections.auditLogs.insertOne({
                timestamp: new Date(),
            ...
            console.log(`👋 Déconnexion loggée pour: ${username}`);
        }
        ...
        await new Promise((resolve, reject) => {
            req.session.save(async (err) => {
                if (err) {
                    ...
                    // ✅ CORRECTION CRITIQUE: Stocker le sessionID dans l'utilisateur
                    ...
                    console.log(`✅ SUCCÈS CONNEXION SUPER ADMIN: ${username} depuis ${req.ip} via /api/admin-login (sessionID: ${req.sessionID})`);
                    resolve();
                }
            });
        });
